...
parser = argparse.ArgumentParser(description="Randomly extract ground truth segments from video sessions.")
...
parser.add_argument("--description_vlm_model", default="gpt-4.1-mini", help="VLM model used for description generation")
...
parser.add_argument("--middle_sessions", default="3-5", help="Range of middle session indices (e.g., '4-6')")
parser.add_argument("--late_sessions", default="6-10", help="Range of late session indices (e.g., '7-10')")
...
activity_predictions_base_dir = f"{base_results_dir}/{participant}/activity_predictions_{window_size}_{sliding_window_length}/{description_vlm_model}/{merging_threshold}_control_{clustering_vlm_model}/{direction}"
...
def sample_from_session_group(sessions, num_samples, group_name, window_size, min_session_duration, activity_predictions_base_dir, sample_id_counter):
    ...
    for session in valid_sessions:
        ...
        # Load ensemble predictions for this session
        df_predictions = load_ensemble_predictions(activity_predictions_base_dir, session_key)
        
        if df_predictions.empty:
            if debug:
                print(f"No ensemble predictions found for session {session_key} in {group_name} group")
            continue
        
        # Filter predictions for this specific session
        # get the predictions for the session right after the current session
        predicted_session_key = session["predicted_session_key"]
        session_predictions = df_predictions[df_predictions['session_key'] == predicted_session_key].copy()
        
        if session_predictions.empty:
            if debug:
                print(f"No predictions found for session {session_key} in {group_name} group")
            continue
        ...
        for _, row in session_predictions.iterrows():
            all_session_timestamps.append({
                ...
                'predicted_session_key': predicted_session_key,
    ...
    for idx in selected_indices:
        ...
        sample_info = {
            ...
            "training_session_key": timestamp_info['training_session_key'],
            ...
            "labels": [f"pred_{timestamp_info['prediction']}", f"gt_{timestamp_info['gt_label']}", f"temporal_{group_name}"]
        }
...
def generate_temporal_stratified_samples(collected_sessions, session_prefix, early_range, middle_range, late_range, 
    ...
    early_samples_list, sample_id_counter = sample_from_session_group(
        grouped_sessions['early'], early_samples, 'early', window_size, min_session_duration, activity_predictions_base_dir, sample_id_counter
    )
    all_samples.extend(early_samples_list)
    
    middle_samples_list, sample_id_counter = sample_from_session_group(
        grouped_sessions['middle'], middle_samples, 'middle', window_size, min_session_duration, activity_predictions_base_dir, sample_id_counter
    )
    all_samples.extend(middle_samples_list)
    
    late_samples_list, sample_id_counter = sample_from_session_group(
        grouped_sessions['late'], late_samples, 'late', window_size, min_session_duration, activity_predictions_base_dir, sample_id_counter
    )
    ...
    "samples": [
        {
            ...
            "training_session_key": sample["training_session_key"],
...
for sample in prediction_samples:
    ...
    sample_filename = f"sample--{str(sample_id).zfill(3)}--{temporal_group}--{training_session_key}--{end_time.strftime('%Y%m%d_%H%M%S')}--{clean_prediction}--{confidence:.3f}"
    if clean_gt_label:
    ...
    print(f"[{sample_id+1}/{len(prediction_samples)}] Processed {temporal_group} sample from {prediction_session_key} (training session: {training_session_key}): {start_time.strftime('%H:%M:%S')} - {end_time.strftime('%H:%M:%S')} (pred: {prediction}, conf: {confidence:.3f})")
