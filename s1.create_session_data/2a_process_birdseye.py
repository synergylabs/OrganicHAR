# in this file, we used sessions created by watch to process the video data from birdseye as well as depth cameras, and save in video_processed folder
import json
import os
import re
import cv2
...
def format_video_files(folder_path: str, sessions: Dict[str, Dict[str, str]], output_dir) -> Dict[str, str]:
    """
    ...
    """
    # convert all video files to mp4 if they are not already
    ...
    for file in os.listdir(folder_path):
        if file.endswith('.mkv'):
            ...
            convert_to_mp4(mkv_path, mp4_file_dir)
        elif file.endswith('.mp4'):
            mp4_path = os.path.join(folder_path, file)
            if not os.path.exists(os.path.join(mp4_file_dir, file)):
                os.rename(mp4_path, os.path.join(mp4_file_dir, file))
    # Update folder_path to the converted mp4 directory
    folder_path = mp4_file_dir
    ...
    for session_name, session_times in sessions.items():
        ...
        for file in sorted_files:
            ...
            try:
                # Get video metadata
                fps, duration, _, _, _ = get_video_metadata(video_path)
                ...
                if file_end_time > start_time and file_time < end_time:
                    session_files.append((file, file_time, file_end_time, fps))
        ...
        first_video = os.path.join(folder_path, session_files[0][0])
        try:
...
if __name__ == "__main__":
    ...
    birdseye_folder = f"{base_raw_dir}/{participant}/vision/birdseye/"
    sessions = {
    ...
    output_dir = os.path.join(participant_processed_dir, "processed_video_data")

    processed_videos = format_video_files(birdseye_folder, sessions, output_dir)
